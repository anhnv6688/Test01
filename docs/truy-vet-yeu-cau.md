...
| VM-10 | ĐÃ CHỐT 17/9/2026: thêm mốc "đã vững" cho từng yêu cầu cần đạt | Đã thực hiện ở `domain/moc-vung.ts`, hiện ở bề mặt phụ huynh (bảng đầy đủ tại `/phu-huynh/lich-su`, tin mừng trong bản tin tối). Suy hoàn toàn từ lịch sử đã có, **không thêm bảng nào** — không có chỗ thứ hai để lệch với sự thật, và không có gì thêm phải xóa khi hộ yêu cầu xóa dữ liệu. Hai ngưỡng (vào ≥80%, ra <60%) để mốc không bật tắt mỗi ngày với trẻ dao động quanh ngưỡng. **Tuyệt đối không hiện trên bề mặt của trẻ** — mốc vững là một dạng đếm điểm, mà đếm điểm bị cấm ở đó theo BR-06 và NT-02; có bài kiểm thử quét `src/app/be`. Hai con số ngưỡng do đội phát triển đặt, **chưa kiểm chứng trên trẻ thật** |
| VM-09 | ĐÃ CHỐT 17/9/2026: bỏ hẳn dải báo "BẢN THỬ" trên mọi trang | Đã gỡ `components/DaiBaoBanThu.tsx` và phần canh nó trong `npm run kiem-moi-truong`; dòng tương ứng trong bảng bất di bất dịch của `CLAUDE.md` cũng bỏ. **Hệ quả phải biết:** lời dặn "đừng nhập tên thật, đừng chụp bài thật của con" và câu nói rõ ảnh có bị gửi ra ngoài hay không nay KHÔNG còn ở đâu trong sản phẩm — chúng chỉ còn là một bước dặn miệng, chép ở `docs/vps-contabo.md`. Máy chủ bản thử đặt ngoài Việt Nam (Nghị định 53), nên rủi ro người test chụp bài thật của con lên đó không còn lớp chặn kỹ thuật nào. Muốn dựng lại thì khôi phục thành phần ấy và mục `kiemDaiBaoBanThu` trong bộ soi |
